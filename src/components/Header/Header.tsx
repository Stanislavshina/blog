...
const Header: React.FC = () => {
  ...
  const btnGroup = isAuth ? (
    <ButtonGroup>
      ...
      <Button typeButton={'bordered'} onSubmit={onLogout} linkType={'simple'}>Log Out</Button>
    </ButtonGroup>
  ) : (<ButtonGroup>
    <Button typeButton={'simple'} to={'/sign-in'}  linkType={'simple'}>Sign In</Button>
    <Button typeButton={'primary'} to={'/sign-up'}  linkType={'simple'}>Sign Up</Button>
   </ButtonGroup>);


  return (
    <HeaderStyled>
       <Link to={'/'} type='simple'>Realworld Blog</Link>
       {btnGroup}
    </HeaderStyled>
  );
};
